...
def dirReduc(arr: List[str]) -> List[str]:
    opposite_part_world = {"NORTH": "SOUTH", "SOUTH": "NORTH", "EAST": "WEST", "WEST": "EAST"}
    result = []

    for part_world in arr:
        if result and opposite_part_world[part_world] == result[-1]:
            result.pop()
            continue

        result.append(part_world)
